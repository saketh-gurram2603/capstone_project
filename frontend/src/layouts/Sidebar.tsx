...
// ── Regular user navigation (no data upload — admin only) ─────────────────────
const USER_NAV = [
  { to: '/',          label: 'Incident Search', icon: Search,        badge: 'Core'  },
  { to: '/triage',    label: 'Agent Triage',    icon: GitBranch,     badge: 'L1→L3' },
  { to: '/analytics', label: 'Evaluation',      icon: BarChart2,     badge: null    },
  { to: '/chat',      label: 'Chat Assistant',  icon: MessageSquare, badge: null    },
]

// ── Admin navigation (data upload + feedback review only) ─────────────────────
const ADMIN_NAV = [
  { to: '/ingest', label: 'Data Upload',     icon: Upload,      badge: null },
  { to: '/admin',  label: 'Feedback Review', icon: ShieldCheck, badge: null },
]

// Routes that belong to the admin area
const ADMIN_PATHS = ['/admin', '/ingest']
...
export default function Sidebar() {
  ...
  // Switch nav context based on current route
  const isAdminArea = ADMIN_PATHS.some(p => pathname.startsWith(p))
  const activeNav   = isAdminArea ? ADMIN_NAV : USER_NAV
  const sectionLabel = isAdminArea ? 'Admin' : 'Workspace'

  function NavItem({ to, label, icon: Icon, badge }: typeof activeNav[number]) {
    ...
    return (
      <NavLink key={to} to={to} end={to === '/'}>
        ...
        >
          ...
          {/* Collapsed-mode pending dot for Feedback Review */}
  ...
  return (
    ...
    >
          ...
          {!collapsed && (
            ...
            >
              ...
              <p className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>
                {isAdminArea ? 'Admin Console' : 'AI Operations'}
              </p>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* ── Nav ───────────────────────────────────────────── */}
      <nav
        className="flex-1 py-4 space-y-0.5 overflow-hidden"
        style={{ padding: collapsed ? '1rem 0.375rem' : '1rem 0.5rem' }}
      >
        <AnimatePresence>
          {!collapsed && (
            <motion.p
        ...
        {!collapsed && (
          <div className="px-3 mb-2 flex items-center gap-1.5">
            ...
            <span className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>
              v1.0.0 · {isAdminArea ? 'admin' : 'development'}
            </span>
          </div>
        )}
        <button
